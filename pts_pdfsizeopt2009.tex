...
\textt{\hbox{<}</Integer 5 /Real -6.7 /String ((C)2009\textbackslash))
/StringHex <Face>
/Boolean true /Name /Foo /List [3 4 5]\hnox{>}>} defines a dictionary
...
i.e.\ defined as an object, and then referred to.
...
uncompressed using PDF's ZIP implementation. The three most common uses for
...
different operators. For example, \texttt{BT/F 20 Tf 1 0 0 1 9 9  
Tm(Hello world)Tj ET} in a content stream draws the text ``Hello World''
with the font \texttt{/F}...
...
It is possible to define a set of objects. 

!! ASCII-repr string hex shorter
!! ASCIIHexDecode shorter
!! why is content stream compression good?

!! base 14: cp `kpsewhich pdftex_ndl14.map` pdftex.map
