...
* accented characters (such as á and ő). These characters won't work (or it
  ...
  Accented characters in PDF filename could be made work the following way.
  pdfsizeopt.exe should call the 16-bit API (GetCommandLineW) instead of
  the single-byte API (GetCommandLineA) to start the Python code. If
  sys.argv doesn't work for passing accented characters, then pdfsizeopt.exe
  should do some escaping. The Python code should use Unicode literals to
  open files (open(u'...')) on Windows. The Python code mustn't pass
  pathnames with non-ASCII characters to the helper programs (e.g.
  Ghostscript, sam2p), because os.system and os.popen work with ASCII only
  in Python. Restricting to ASCII is feasible, because the filenames passed
  in helper program command lines are all autogenerated temporary filenames.
  Please note that filenames would still be displayed incorrectly in the
  Command Prompt window (because Python can't use the 16-bit API to
  write to the console).

  Accented characters will probably not work in the full pathname of
