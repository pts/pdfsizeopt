...
# TODO(pts): Try to recompress already compressed streams, maybe save bytes.
...
class PdfData(object):
  ...
  def Save(self, file_name, display_file_name, multivalent_java,
           do_update_file_meta,
...
def main(argv, script_dir=None, zip_file=None):
  ...
  elif (libexec_dir is not None and
  ...
  if f.do_decompress_most_streams:
    ...
    # TODO(pts): Also decompress in Multivalent output.
    pdf.DecompressStreams(is_flate_only=True)
  if f.do_compress_uncompressed_streams and multivalent_java is None:
