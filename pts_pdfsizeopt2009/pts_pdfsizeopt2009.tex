...
% * stretch tables to \hsize
% * recompile sam2p.exe etc.
%
% !! TODO: fix \MF and \MP in the optimized PDF
% !! a másodikban kicsit zavaros, hogy melyik 
%    tömörítési eljárást ki és hogyan végzi. Eleinte leírod, hogy nem kell
%    törõdni vele, pdfsizeopt.py megcsinálja, késõbb az olvasó bizonytalanságban marad
% !! Az 5. táblázattól kezdve mindegyikben vannak kurzív számok, miért,
%    esetleg a caption-ban is meg lehetne/kéne magyarázni.
...
make sure that we'll have working converter at restore time. With
...
A pixel-based (fixed resolution) alternative of PDF is DjVu (see in
...
the dictionary specify how to uncompress/\allowbreak decode the stream data.
It possible the specify multiple such filters,
...
Tm(Hello world)\allowbreak Tj ET} in a content stream draws the text ``Hello World''
...
optimizer tool. The approach we suggest in this work a mixture of the two:
...
Acrobat. \cmd{ps2pdf}
(using Ghostscript's \texttt{-dDEVICE=pdfwrite}, use
...
\cite{pngout}, see OptiPNG \cite{optipng}, see also \cite{optipng-article}
for a list of 11
other PNG optimization tools) and others at \cite{png-recompressors})
...
linearized PDF. It is save to save space by removing those unused objects. A
...
PDF supports storing old object versions in the file. This makes incremental
...
images created with codeMantra Universal PDF \cite{codemantra}. Font data is a mixture of
...
\texttt{java -cp Multivalent20060102.jar tool.pdf.Compress doc.pdf}. This
...
Ghostscript is used for parsing CFF to the font dictionary data structure,
and also for serializing the data structure as CFF. Again, the latter is
done by generating a PostScript file with all the fonts, then converting it
to a PDF using Ghostscript.
Limitations: itt only works for CFF (and former Type\,1) fonts;
...
This step is needed, because most tools (including \cmd{pdfsizeopy.py})
don't optimize inline images.
...
First the image data is decompressed (with
Ghostscript if the image data is compressed with anything other than simple
ZIP), then it is recompressed with maximum-effort ZIP, then it is converted
...
with the smalles PDF image object representation, counting the stream
...
The implementation uses equivalence classes, so duplicate subtrees referring
...
All objects not reachable from the trailer object are removed.
...
\cmd{pdfsizeopt.py} can count the total number of bytes occupied by the
...
create the PDF using pdf\TeX{} or \cmd{dvipdfmx}, using the proper
...
concatenate with Ghostscript, because that blows up the file size.
...
PNG image optimization \cite{png-recompressors,optipng,pngout}
!!
...
Multivalent supports a custom file format named \emph{compact PDF,}
...
!! more related work; also other \cite{}s from this document
...
those created by \cmd{sam2p}, and not only in form XObjects). Also
\cmd{pdfsizeopt.py} would benefit from compiling a test set of PDF, and
